...
def render_khbd_ui(is_ai_enabled: bool = True):
    ...
    # GOM CỤM TOÀN BỘ VÀO TRONG 1 FORM để bảo vệ luồng bộ nhớ file tải lên
    ...
    # ---------------------------------------------------------
    # LUỒNG XỬ LÝ KHI BẤM NÚT KÍCH HOẠT (ĐỨNG NGOÀI FORM)
    # ---------------------------------------------------------
    if submit_button:
        if not ten_bai.strip():
            ...
            with st.spinner(f"⏳ Đang trích xuất file và gọi xử lý từ mô hình [{model_name}]..."):
                try:
                    # Đọc nóng trực tiếp dòng byte ngay khi submit để không bao giờ bị mất dữ liệu nguồn
                    extracted_sgk_text = "\n".join([safe_extract_file(f) for f in file_sgk]) if file_sgk else ""
                    extracted_ga_text = "\n".join([safe_extract_file(f) for f in file_ga]) if file_ga else ""

                    sgk_safe_text = extracted_sgk_text[:120000]
                    if len(extracted_sgk_text) > 120000:
                        sgk_safe_text += "\n\n[...NỘI DUNG ĐÃ BỊ CẮT BỚT DO FILE QUÁ DÀI...]"
                    
                    ga_safe_text = extracted_ga_text[:40000]
    ...
    # ĐỌC TRẠNG THÁI HIỂN THỊ (PREVIEW & DOWNLOAD)
